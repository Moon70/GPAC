 ...
 */
public class SierraDithering implements IDitheringAlgorithm{
	...
	public void dither(int[] pixeldata,int width,int height,Palette palette,ArrayList<ICube> cubes) {
		...
		for(int y=0;y<height;y++) {
			for(int x=0;x<width;x++) {
				...
				if(x<width-1) {
					pixeldataR[(y)*width+x+1]+=deltaRed*5;
					pixeldataG[(y)*width+x+1]+=deltaGreen*5;
					pixeldataB[(y)*width+x+1]+=deltaBlue*5;
					if(y<height-1) {
				...
				if(y<height-2) {
					pixeldataR[(y+2)*width+x]+=deltaRed*3;
					pixeldataG[(y+2)*width+x]+=deltaGreen*3;
					pixeldataB[(y+2)*width+x]+=deltaBlue*3;
					if(x>0) {
						pixeldataR[(y+2)*width+x-1]+=deltaRed<<1;
						pixeldataG[(y+2)*width+x-1]+=deltaGreen<<1;
						pixeldataB[(y+2)*width+x-1]+=deltaBlue<<1;
